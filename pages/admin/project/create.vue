        ...
        <div class="flex gap-10 w-full">
          ...
          <div class="w-1/2 pr-5 flex flex-col gap-4">
            ...
            <div class="relative">
              ...
              <input type="text" class="input_style"   @input="selectClient"  placeholder="Client's @username" />
              <!-- select option -->
              <select class="input_style" v-model="form.value.clientUsername">
  ...
  if (!authStore.user) {
    authStore.fetchUser().then(() => {
      if (user.value && user.value._id) {
        ...
        form.value.clientUsername = user.value.username;
...
const selectClient = async (event) => {
  ...
  console.log('Selected client username:', username);
  ...
  try {
    ...
    if (data && data._id) {
      form.value.clientUsername = data.username;
      showAlert({ type: 'success', value: `Client selected: ${data.username}` });
    } else {
      showAlert({ type: 'error', value: 'Client not found' });
    }
  } catch (error) {
    console.error('Error fetching client:', error);
    showAlert({ type: 'error', value: 'Error fetching client' });
  }
};
